...
class KDTree
{
  ...
  public:
    ...
    void build_tree(vector<Point<Dim>> points, KDTreeNode *& root, int d);

    // copy helper function
    void _copy(const KDTreeNode *& other, KDTreeNode *& curr);
    // destroy helper function
    void _destroy(KDTreeNode *& root);

    // find nearest neighbor helper function
    Point<Dim> * _find(const Point<Dim> & query, Point<Dim> * currBest, KDTreeNode * currNode, int d) const;
